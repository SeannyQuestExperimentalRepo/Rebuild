...
const nflAngles = executeReverseLookup({
  sport: "NFL",
...
const ncaambAngles = executeReverseLookup({
  sport: "NCAAMB",
...
const weatherAngles = executeReverseLookup({
  sport: "NFL",
  seasonRange: [2015, 2024],
  categories: ["weather"],
  minStrength: "noise",
...
const chiefsAngles = executeTeamReverseLookup("NFL", "Kansas City Chiefs", [2018, 2024], 10);
assert(
...
const allSportsAngles = executeReverseLookup({
...
const strongOnly = executeReverseLookup({
  sport: "NFL",
...
const chiefsForSig = executeTrendQuery(chiefsQuery);
...
const emptyForSig = executeTrendQuery(emptyQ);
...
const dailyContext = getDailyGameContext("2024-09-05", "NFL");
assert(
...
const matchup = getMatchupContext("NFL", "Kansas City Chiefs", "Buffalo Bills");
...
// Test empty date
const emptyDate = getDailyGameContext("2030-01-01", "NFL");
assert(emptyDate.games.length === 0, `No games on future date`);

// Test a date with multiple games
const bigGameDay = getDailyGameContext("2024-09-08", "NFL");
assert(
